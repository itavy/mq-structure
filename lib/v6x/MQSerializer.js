...
class MQSerializer {
  ...
  serialize(request, version = '1') {
    return new Promise((resolve) => {
      ...
      const msgToSerialize = serializer.fromObject(
        Object.assign(request.toJSON(), {
          msgType: schema,
        }));
      this.debug('msgToSerialize:', msgToSerialize);
      return resolve(
        serializer
          .encode(msgToSerialize)
          .finish());
    });
  }
  ...
  unserialize(request) {
    return this.getUnserializer(request)
      .then((unserializeInfo) => {
        ...
        const builder = unserializeInfo.builder;
        const message = unserializer.toObject(
          unserializer.decode(request), { defaults: true, longs: Number });
  ...
  getUnserializer(request) {
    return new Promise((resolve, reject) => {
      ...
      ({ builder, schema, serializer } = this.messagesVersion.get(
